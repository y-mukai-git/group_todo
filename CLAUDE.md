...
**ユーザーから指示が来たら**：
1. **必ずWORK_CHECKLIST.mdを実行**（例外なし）
2. **ファイル修正時は必ず事前許可**（1行でも許可必須）
...
1. **許可なしでファイル修正**
   - 「軽微」「明らか」でも許可必須
   - 読み取り・確認のみは許可不要
   - **例外：セッションログ記録（`session_logs/YYYY-MM-DD.md`）は事前許可不要**

2. **git操作**
   - checkout, reset, stash, revert等すべて禁止

3. **テストデータ使用**
   - test_, dummy_, sample_, 仮の 等禁止
...
**記録場所**：
- 必ず当日日付ファイル `session_logs/YYYY-MM-DD.md`
- 過去日付ファイルへの記録は禁止
