...
-- 作成日: 2025-11-01
-- 目的: グループ招待時の承認フロー実装
--       - 招待時にロール（owner/member）を選択可能
--       - 招待された側が承認して初めてグループ参加
--       - 複数オーナー制によるオーナー不在問題の解決
...
BEGIN
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Group Invitations Migration Completed';
  RAISE NOTICE '========================================';
  RAISE NOTICE 'Table Created: group_invitations';
  RAISE NOTICE 'Indexes: 4 indexes created';
  RAISE NOTICE 'RLS Policies: 4 policies created';
  RAISE NOTICE '  - SELECT: inviter or invited user';
  RAISE NOTICE '  - INSERT: group owner only';
  RAISE NOTICE '  - UPDATE: invited user only';
  RAISE NOTICE '  - DELETE: inviter only';
  RAISE NOTICE '========================================';
END $$;
